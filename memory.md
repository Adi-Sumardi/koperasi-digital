...
* **Belum dikerjakan**: 2FA TOTP untuk Super Admin & persetujuan pencairan besar (rules/security.md §1 — fitur terpisah yang cukup besar, sengaja belum digarap), audit log (`AUDIT_LOGS` cuma relasi di ERD, belum ada tabel/implementasi), dan halaman manajemen anggota/simpanan/produk pinjaman lain di luar KYC & approval pinjaman (di luar scope yang diminta).
